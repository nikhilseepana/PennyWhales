...
const DB_FILES = {
  ...
  usDailyMiniHistory: path.join(__dirname, 'data', 'usDailyMiniHistory.json')
};
...
class DatabaseService {
  ...
  async init() {
    ...
    // Initialize US daily mini history database
    const usDailyMiniHistoryAdapter = new JSONFile(DB_FILES.usDailyMiniHistory);
    this.dbs.usDailyMiniHistory = new Low(usDailyMiniHistoryAdapter, {});
    await this.dbs.usDailyMiniHistory.read();
    this.dbs.usDailyMiniHistory.data = this.dbs.usDailyMiniHistory.data || {
      entries: []
    };
    await this.dbs.usDailyMiniHistory.write();
  ...
  // US Daily Mini History Management
  async addUsDailyMiniSnapshot(snapshot) {
    await this.init();

    if (!this.dbs.usDailyMiniHistory.data || !Array.isArray(this.dbs.usDailyMiniHistory.data.entries)) {
      this.dbs.usDailyMiniHistory.data = { entries: [] };
    }
    ...
    const entry = {
      id: `us_daily_mini_${Date.now()}`,
      timestamp: new Date().toISOString(),
      scanDate: snapshot.scanDate || new Date().toISOString().slice(0, 10),
      sourceUrl: snapshot.sourceUrl || null,
      totalMatched: normalizedTickers.length,
      totalAfterRejectedFilter:
        typeof snapshot.totalAfterRejectedFilter === 'number'
          ? snapshot.totalAfterRejectedFilter
          : normalizedTickers.length,
      tickers: normalizedTickers,
      notes: snapshot.notes || null,
    };

    this.dbs.usDailyMiniHistory.data.entries.push(entry);

    // Keep only the latest 365 snapshots
    if (this.dbs.usDailyMiniHistory.data.entries.length > 365) {
      this.dbs.usDailyMiniHistory.data.entries = this.dbs.usDailyMiniHistory.data.entries.slice(-365);
    }

    await this.dbs.usDailyMiniHistory.write();
    console.log(`🗂️ Saved US daily mini snapshot (${entry.totalMatched} tickers)`);
    return entry;
  }

  async getUsDailyMiniHistory(limit = 60) {
    await this.init();
    const entries = this.dbs.usDailyMiniHistory.data?.entries || [];
    const cappedLimit = Math.max(1, Math.min(Number(limit) || 60, 365));
